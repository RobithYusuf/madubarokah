...
class PesananController extends Controller
{
    ...
    /**
     * Update order status dengan sinkronisasi otomatis ke 3 tabel
     * Method ini menggantikan 3 dropdown dengan 1 aksi terpusat
     */
    public function updateOrderStatus(Request $request, $id)
    {
        $request->validate([
            'action' => 'required|in:process_payment,process_order,ship_order,complete_order,cancel_order'
        ]);

        try {
            $pesanan = Transaksi::with(['pembayaran', 'pengiriman'])->findOrFail($id);
            ...
            switch ($request->action) {
                case 'process_payment':
                    ...
                    $pesanan->pengiriman->update(['status' => 'diproses']);
                ...
                case 'process_order':
                    ...
                    if (!in_array($pesanan->status, ['dibayar', 'berhasil'])) {
                        throw new \Exception('Pesanan belum dibayar');
                    }
                    
                    $pesanan->update(['status' => 'berhasil']);
                    $pesanan->pengiriman->update(['status' => 'diproses']);
                ...
                case 'ship_order':
                    // Kirim pesanan (dengan resi)
                    if (!in_array($pesanan->status, ['dibayar', 'berhasil'])) {
                        throw new \Exception('Pesanan belum siap dikirim');
                    }
                    
                    if (!$request->resi) {
                        throw new \Exception('Nomor resi harus diisi');
                    }
                    ...
                    $message = 'Pesanan berhasil dikirim dengan resi: ' . $request->resi;
                    break;
                    
                case 'complete_order':
                    // Selesaikan pesanan
                    if ($pesanan->status !== 'dikirim') {
                        throw new \Exception('Pesanan belum dalam status dikirim');
                    }
                    
                    $pesanan->update(['status' => 'selesai']);
                    $pesanan->pengiriman->update(['status' => 'diterima']);
